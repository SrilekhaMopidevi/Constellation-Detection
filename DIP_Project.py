...
def makeTemplates():        
	...
	template_directory = sys.path[0]+"/Templates"
	...
	for filename in os.listdir(template_directory):
		...
		img = cv2.imread(sys.path[0]+"Templates/" + filename + ".png")
...
def test_runner(constellation) : 
	test_coordinates = test_normaliser(sys.path[0]+'/test_data/' + constellation + '.png')
	
	file = open('/home/srilekha/Desktop/Workspace/Major Project/DetectionOfConstellation/Constellation-detection/Template Coordinates' , 'rb')
	...
	plt.figure('Matched ' + constellation + " " + pred_label	)
	# print('PPS: ', plot_points)
	...
	for line in plot_points[3]:
		for x1,y1,x2,y2 in line:
			plt.plot([x1, x2], [y1, y2], color='red')
	#if (pred_label==constellation):
	plt.savefig("/home/srilekha/Desktop/Workspace/Major Project/DetectionOfConstellation/Constellation-detection/Predicted_images/" + pred_label)
	plt.close()
...
def detection():
	d = ['Aquila', 'Auriga' ,'CanisMajor' ,'Columba' , 'Gemini' , 'Grus' , 'Leo' , 'Orion' ,'Phoenix','Vela'] #['Andromeda','Aquila', 'Auriga' , 'CanisMajor' , 'Capricornus' , 'Cetus' , 'Columba' , 'Gemini' , 'Grus' , 'Leo' , 'Orion' , 'Pavo' , 'Pegasus' , 'Phoenix' , 'Pisces' , 'PiscisAustrinus' , 'Puppis' , 'UrsaMajor' , 'UrsaMinor' , 'Vela']
	count = 0
	for i in d :
		pred = test_runner(i)
		if (pred == i) :
